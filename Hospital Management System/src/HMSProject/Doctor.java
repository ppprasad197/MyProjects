...
public class Doctor {
	...
	public void addDoctor() {
		String query = "insert into doctors(name,specialisation)values(?,?)";
	...
	public void deleteDoctorById() {
		...
		try {
			...
			if (isDeleted > 0) {
				...
				System.out.println("No doctor present against that ID");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void updateDoctorById() {
		String query = "UPDATE doctors SET name = ?, specialisation = ? WHERE id = ?";
		Scanner sc = new Scanner(System.in);

		System.out.println("Enter doctor's ID: ");
		int doctorId = sc.nextInt(); // Store the ID before checking
		...
		if (getDoctorById(doctorId)) {
			try {
				...
				System.out.println("Enter doctor's updated name: ");
				String updatedName = sc.nextLine();

				System.out.println("Enter doctor's updated specialisation: ");
				String updatedSpecialisation = sc.nextLine();

				preparedStatement.setString(1, updatedName);
				preparedStatement.setString(2, updatedSpecialisation);
				preparedStatement.setInt(3, doctorId);

				int rowsUpdated = preparedStatement.executeUpdate();

				if (rowsUpdated > 0) {
					System.out.println("Doctor's record updated successfully!");
				} else {
